    ...
    <div className="p-8 max-w-6xl">
      ...
      <div className="card-lit border border-border/40 rounded-md p-7 mb-6 relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-5 border-t border-border/30">
          ...
          {contact.email ? (
            <Stat label="Email" customValue={
              <a href={`mailto:${contact.email}`} className="font-display text-base tracking-wider truncate text-primary hover:text-primary/80 hover:underline block">
                {contact.email}
              </a>
            } icon={Mail} />
          ) : (
        ...
        {(contact.account || contact.linkedin_url) && (
          <div className="mt-5 pt-5 border-t border-border/30 flex items-center gap-6 flex-wrap">
            {contact.account && (
              <div>
                ...
                <Link href={`/accounts/${contact.account.id}`} className="inline-flex items-center gap-2 hover:text-primary transition-colors">
        ...
        <div className="space-y-6">
          <div className="card-lit border border-border/40 rounded-md p-6 relative">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href={`/deals/new?contact=${id}${contact.account_id ? `&account=${contact.account_id}` : ''}`} className="text-[10px] uppercase tracking-[0.15em] text-primary hover:text-primary/80">
                + Add
              </Link>
            </div>
