...
export default async function DealDetailPage({
  ...
  return (
    <div className="p-8 max-w-6xl">
      <Link href="/deals" className="inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.15em] text-muted-foreground hover:text-foreground mb-6 transition-colors">
        ...
        <div className="flex items-start justify-between gap-4 mb-5">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              {deal.account && (
                <Link href={`/accounts/${deal.account.id}`} className="text-xs hover:text-primary">
                  {deal.account.name}
                </Link>
              )}
          ...
          {deal.owner && (
            <div className="mt-5 pt-5 border-t border-border/30">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">{deal.owner.full_name || deal.owner.email}</span>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div>
      ...
      {customValue ? (
        ...
        <div className={`font-display text-base tracking-wider truncate ${
          highlight ? 'text-primary text-glow-primary text-2xl' : warn ? 'text-destructive' : ''
        }`}>
          {value}
        </div>
      )}
