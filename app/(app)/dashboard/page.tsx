...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('activities')
      ...
      .eq('type', 'task').eq('assigned_to', profile.id).is('completed_at', null)
      .order('scheduled_at', { ascending: true, nullsFirst: false }).limit(5),
    supabase.from('activities')
      .select('id, type, subject, scheduled_at, account:accounts(id, name), contact:contacts(id, first_name, last_name), deal:deals(id, name)')
      .eq('owner_id', profile.id).is('completed_at', null)
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl">
      <div className="mb-8 md:mb-10">
        ...
        <h1 className="font-display text-4xl md:text-6xl tracking-wider leading-none">
          WELCOME BACK,{' '}
      ...
      <div className="grid md:grid-cols-2 gap-5 md:gap-6">
        <DashboardCard title="My tasks" icon={CheckSquare} href="/activities" emptyText="No open tasks. Nice.">
          {(myOpenTasks || []).map(t => (
            <Link key={t.id} href={
              ...
              className="block py-2 px-2 rounded-md hover:bg-primary/5 transition-colors -mx-2 min-h-[44px]"
            >
        ...
        <DashboardCard title="On your calendar today" icon={Calendar} href="/activities" emptyText="Nothing scheduled today.">
          {(upcomingToday || []).map(a => (
            <div key={a.id} className="py-2 px-2 -mx-2 min-h-[44px]">
  ...
  return (
    ...
    >
      ...
      <div className="p-4 md:p-5">
        ...
        <div className="font-display text-2xl md:text-3xl tracking-wider leading-none">{value}</div>
    ...
    <div className="card-lit text-card-foreground rounded-md border border-border/40 relative">
      ...
      <div className="p-5 md:p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href={href} className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground hover:text-primary min-h-[32px] inline-flex items-center">
            View all
          </Link>
        </div>
        {hasChildren ? (
