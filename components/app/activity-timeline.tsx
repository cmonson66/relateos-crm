...
export function ActivityTimeline({
  ...
  if (!activities.length && !auditEntries.length) {
    return (
      <div className="card-lit border border-border/40 rounded-md p-6 relative">
        ...
        <p className="text-sm text-muted-foreground italic">No activity yet. Log a call, note, or schedule a task above.</p>
      </div>
    );
  }
  ...
  // Build merged timeline (excluding upcoming)
  ...
  return (
    <div className="card-lit border border-border/40 rounded-md p-6 relative">
      ...
      {upcoming.length > 0 && (
        <>
          <h2 className="font-display text-xl tracking-wider mb-4">UPCOMING · {upcoming.length}</h2>
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        ...
        {activity.subject && (
          <div className="text-sm font-medium mb-0.5">{activity.subject}</div>
        )}
        {activity.body && (
          <div className="text-sm text-muted-foreground whitespace-pre-wrap">{activity.body}</div>
        )}
        ...
        {upcoming && (
          <button
            ...
            onClick={onComplete}
            disabled={pending}
            className="mt-2 inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.15em] text-primary hover:text-primary/80 transition-colors"
          >
...
function AuditRow({ entry }: { entry: AuditEntry }) {
  // actor may come back as a single object or an array depending on Supabase relations
